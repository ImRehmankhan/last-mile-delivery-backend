...
{
    // Fetch raw data
    $rawDetails = DB::table('orderdetails')
        ->join('itemdetails', 'orderdetails.itemdetails_ID', '=', 'itemdetails.id')
        ->join('items', 'itemdetails.item_ID', '=', 'items.id')
        ->leftJoin('itemattributes', 'itemdetails.id', '=', 'itemattributes.itemdetail_id')
        ->select(
    ...
    // Group by order_detail_id to combine attributes
    $grouped = $rawDetails->groupBy('order_detail_id');

    $orderDetails = $grouped->map(function ($items, $orderDetailId) {
        $first = $items->first(); // all shared values
        $attributes = $items->map(function ($item) {
            ...
            return $attr['key'] !== null; // remove null attributes
        })->values();

        return [
            'order_detail_id' => $orderDetailId,
            'quantity' => $first->quantity,
            'order_detail_price' => $first->order_detail_price,
            'order_detail_total' => $first->order_detail_total,
            'item' => [
                'item_id' => $first->item_id,
                'item_name' => $first->item_name,
                'item_description' => $first->item_description,
                'item_detail_id' => $first->item_detail_id,
                'variation_name' => $first->variation_name,
                'item_detail_price' => $first->item_detail_price,
                'additional_info' => $first->additional_info,
                'item_picture' => $first->item_picture? asset('storage/' . $first->item_picture) : null,
